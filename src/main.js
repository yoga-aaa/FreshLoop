...
import { ingredientIcon } from './data/ingredientKnowledge.js';
...
function renderFoodRow(item, manageMode) {
  ...
  return `<article class="food-row ${tone}-row"><div class="food-symbol large">${ingredientIcon(item.name)}</div><div class="food-copy"><div class="food-name-line"><h3>${escapeHtml(item.name)}</h3>${expiry ? `<span class="expiry-pill ${tone}">${escapeHtml(expiry)}</span>` : item.stockPercentage <= 20 ? '<span class="expiry-pill pink">建议补货</span>' : ''}</div><p>${escapeHtml(item.category)} · ${escapeHtml(item.storageLocation)}${dateText ? ` · 到期 ${dateText}` : ''}</p><div class="food-hover-note"><span>${escapeHtml(item.story || '')}</span><span>${escapeHtml(item.nature || '')} · ${escapeHtml(item.cooking || '')}</span></div></div><div class="food-amount"><strong class="${item.managementMode === 'freshness_only' ? 'no-quantity' : ''}">${escapeHtml(amount)}</strong><small>${item.managementMode === 'approximate_stock' ? '大致库存' : item.managementMode === 'freshness_only' ? '用完即删' : '已记录'}</small></div>${controls}</article>`;
}
...
function openItemDetail(id) {
  ...
  const modal = createModal(`<form class="item-edit-form" data-item-edit><div class="item-detail"><span class="food-symbol large">${ingredientIcon(item.name)}</span><div class="modal-heading"><p class="eyebrow">INGREDIENT DETAILS</p><h2>${escapeHtml(item.name)}</h2><p>${escapeHtml(item.story || '')}</p></div></div><div class="item-detail-note"><strong>${escapeHtml(item.nature || '')}</strong><p>${escapeHtml(item.cooking || '')}</p></div><div class="item-edit-grid">${amountField}<label>储存方式<select name="storageLocation">${STORAGE_VALUES.map((value) => `<option value="${value}" ${item.storageLocation === value ? 'selected' : ''}>${value}</option>`).join('')}</select></label><label>预计到期日<input name="expiryDate" type="date" value="${escapeHtml(item.expiryDate || '')}"></label></div><div class="modal-actions"><button type="button" class="secondary-button" data-close>取消</button><button type="submit" class="primary-button">保存修改</button></div></form>`);
  modal.querySelector('[data-item-edit]').addEventListener('submit', (event) => {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    store.update((next) => {
